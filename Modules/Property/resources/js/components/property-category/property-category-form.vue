<template>
  <Card>
    <template #header
      >{{ form.id ? "Edit" : "Create" }} Property Category</template
    >
    <form @submit.prevent="handleSubmit">
      <div class="grid grid-cols-12 gap-4">
        ...
        <div class="md:col-span-6 col-span-12">
          <!-- Label -->
          <InputLabel for="icon" value="Property Types" />

          <!-- Multiselect Dropdown -->
          <Multiselect
            v-model="filteredChildren"
            :options="categories"
            name="parent_id"
            :multiple="true"
        ...
        <div class="col-span-12">
          ...
          <textarea
            ...
            class="form-input mt-2 w-full textarea"
...
watch(
  ...
  (newCategory) => {
    ...
    filteredChildren.value = newCategory?.children || [];
  },
  { immediate: true }
);
...
const form = useForm({
  ...
  parent_ids: props.category?.parent_ids || [],
...
watch(
  ...
  (newCategory) => {
    ...
    form.parent_ids = newCategory?.parent_ids || [];
    form.name = newCategory?.name || "";
    form.slug = newCategory?.slug || "";
    form.icon = newCategory?.icon || "";
    form.iconObject =
      props.icons.find((icon) => icon.name === newCategory?.icon) || null;
...
const handleSubmit = () => {
//   const parent_ids = filteredChildren.value.map((category) => category.id); // Extract IDs
//   const payload = {
//     ...form, // Include other form data if necessary
//     parent_ids,
//   };
form.parent_ids = filteredChildren.value.map((category) => category.id);
  form.id
...
const handleCategoryChange = (category) => {
  form.parent_id = category?.id || null;
