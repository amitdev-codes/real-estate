...
<template>
  ...
  >
    ...
    />
    <div v-else-if="viewType === 'custom' && customViewData">
        <!-- Placeholder for custom view logic -->
    </div>
    </div>
</template>
...
const emit = defineEmits(["create", "edit", "export", "show-export-modal","archive"]);
...
const handleArchive = (rowData) => {
  router.get(route(`admin.${props.resourceName}.archive`, rowData.id));
};
...
const initTable = () => {
  ...
  $(dataTableRef.value).off('click', '.btn-archive').on('click', '.btn-archive', function() {
        const rowData = tableInstance.value.row($(this).closest('tr')).data();
        handleArchive(rowData);
    });

  const viewHandler = (rowData, viewType = 'default') => {
        if (viewType === 'default') {
            // Default modal view
            handleView(rowData);
        } else {
...
const fetchCustomViewData = async () => {
    if (props.viewType === 'custom' && props.selectedId) {
        try {
            const response = await axios.get(`/admin/${props.resourceName}/custom-view/${props.selectedId}`);

            // If the backend is set up to return Inertia response, use router.visit
            if (response.data.component) {
                router.visit(route(`admin.${props.resourceName}.custom-view`, { id: props.selectedId }), {
                    preserveState: true,
                    preserveScroll: true
                });
            } else {
                // If it's a regular AJAX response, handle accordingly
                customViewData.value = response.data;
            }
        } catch (error) {
            console.error('Error fetching custom view data:', error);
            // Optionally show error toast or handle error
        }
    }
};

// Watch for changes in selectedId when view type is custom
watch(() => props.selectedId, (newId) => {
    if (props.viewType === 'custom' && newId) {
        fetchCustomViewData();
    }
}, { immediate: true });
