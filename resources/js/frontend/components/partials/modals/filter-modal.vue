  ...
  <Vueform size="md" :display-errors="false">
    ...
    >
      ...
      <!-- <div>
          ...
          <pre>{{ categoryStore.selectedCategory }}</pre>
        </div> -->
      <RadiogroupElement
        ...
        :default="categoryStore.selectedCategory"
...
const selectedParentCategory = ref(categoryStore.selectedCategory);
...
const updateFilteredSubcategories = (parentCategory) => {
    ...
    categoryStore.updateCategory(parentCategory);
...
const handlePropertySizeChange = (value) => {
  ...
  if (value === "Metres²") {
    sliderDefault.value = [300, 10000];
    maxValue.value = 10000;
    step.value = 100;
    propertySizeSlider.value = minValue; // Reset slider value
  } else if (value === "Acres") {
    minValue.value = 0;
    sliderDefault.value = [0, 1000];
    maxValue.value = 1000;
    step.value = 10;
    propertySizeSlider.value = minValue; // Reset slider value
...
const handlePropertySizeUnitChange = (newUnit) => {
  ...
  propertySizeSlider.value = newUnit; // Ensure the slider value changes
};
...
onMounted(async () => {
  ...
  console.log("this is from homepage category selected >>> ", categoryStore.selectedCategory);

  await fetchCategories();
  console.log("Categories after fetch: ", categories.value);

  // Once categories are populated, then access selected category
  if (categories.value && categories.value[selectedParentCategory.value]) {
    console.log(
